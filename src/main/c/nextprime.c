...
int find_primes(unsigned char *, mpz_t, unsigned long, mpz_t);
...
int main(int argc, char *argv[])
{
  ...
  char* stdin_buff = calloc(1, 6050000);  // 6 million digits, enough for 2^20mil
  if (stdin_buff == NULL) {
    return 1;
  }
  read(0, stdin_buff, 6050000);
  mpz_set_str(fr, stdin_buff[2], 0);
  //mpz_set_str(fr, argv[1], 0);
  //mpz_inp_str(fr, NULL, 10);
  if (argv[2][0] == '+')
  {
...
#if DEBUG
    printf("Sieving region [");
    mpz_out_str(stdout, 10, fr2);
    printf(",");
    mpz_add_ui(tmp, fr2, rsize - 2);
    mpz_out_str(stdout, 10, tmp);
    printf("]\n");
#endif
    sieve_region(s, fr2, rsize);
    int found = find_primes(s, fr2, rsize / 2, siev_sqr_lim);
    if (found) {
      return 1;
    }
...
void sieve_region(unsigned char *s, mpz_t fr, unsigned long rsize)
{
...
// 1 = found, 0 = otherwise
int find_primes(unsigned char *s, mpz_t fr, unsigned long ssize,
                 mpz_t siev_sqr_lim)
{
  unsigned long j, ij;
  mpz_t tmp;
...
void make_primelist(unsigned long maxprime)
{
#if 1
  unsigned char *s;
